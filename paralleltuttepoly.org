...
mgraph *
new_mgraph(int n, int m)
{
  DYNALLSTAT(int,lab,lab_sz);
  DYNALLSTAT(graph,ng,ng_sz);

  DYNALLOC1(int,lab,lab_sz,n,"malloc");
  DYNALLOC2(graph,ng,ng_sz,m,n,"malloc");

  mgraph *g = (mgraph *)malloc(sizeof(mgraph));

  g->n = n; g->m = m;
  g->l = lab; g->l_sz = lab_sz;
  g->g = ng; g->g_sz = ng_sz;
  return g;
}
...
print_poly(FILE *fout, poly *p)
{
  for (int x = 0; x < p->x_deg; x++)
    for (int y = 0; y < p->y_deg; y++)
      fprintf(fout, "%ldx^%dy^%d\n", *poly_coeff(p, x, y), x+1, y+1);
}

poly *
poly_add(poly *A, poly *B)
{
  poly *C = (poly *)calloc(1, sizeof(poly));
  C->x_deg = MAX(A->x_deg, B->x_deg);
  C->y_deg = MAX(A->y_deg, B->y_deg);
  C->c = (long *)calloc((C->x_deg)*(C->y_deg), sizeof(long));
...
{
  poly *C = (poly *)calloc(1, sizeof(poly));
  C->x_deg = A->x_deg + B->x_deg;
  C->y_deg = A->y_deg + B->y_deg;
  C->c = (long *)calloc((C->x_deg)*(C->y_deg), sizeof(long));
...
edge select_edge(mgraph *g) {
  ...
  for (int i = 0; i < n; i++)
    {
      set *row = GRAPHROW(g->g, i, g->m);
      for (int j = 0; j < m; j++)
        {
          int fb = FIRSTBIT(row[j]);
          if (fb != WORDSIZE)
            return (edge){ .u = i, .v = fb + j*m };
        }
    }
...
*/
  ...
  mgraph *g_out = new_mgraph(n, m);
  graph *g = g_out->g;


  memcpy((void *)g,(void *)g_in->g, m*(size_t)n*sizeof(setword));
...
mgraph *
delete_edge(mgraph *g_in, edge e) // only handles simple edges
{
  int n = g_in->n; int m = g_in->m;

  mgraph *g = new_mgraph(n, m);
  graph *ng = g->g;

  memcpy((void *)ng,(void *)g_in->g, m*(size_t)n*sizeof(setword));

  int u = e.u; int v = e.v;
  DELELEMENT(GRAPHROW(ng,u,m),v);
  DELELEMENT(GRAPHROW(ng,v,m),u);
